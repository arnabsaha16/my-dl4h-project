...
class FileECGQADataset(RawECGTextDataset):
    ...
    def __getitem__(self, index):
        ...
        answer = data['answer']
        res['answer'] = torch.tensor(answer, dtype=torch.int32) #torch.LongTensor(answer)
        ...
        res["valid_classes"] = torch.tensor(data["valid_classes"][0], dtype=torch.int32)  #torch.LongTensor(data["valid_classes"][0])

        #res["ecg_id"] = [int(x.split("/")[-1].split("_hr")[0]) for x in data["ecg_path"]]

        res["ecg_id"] = [int(os.path.basename(x).split("_hr")[0]) for x in data["ecg_path"]]
        return res
